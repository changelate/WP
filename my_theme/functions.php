...
function my_theme_setup()
{
	...
	function cinema_theme()
	{
		wp_enqueue_style(
			'cinema-theme',
			get_template_directory_uri() . '/page/style.css',
			array(), // например: array('bootstrap')
			'1.0.0',
			'all'
		);
	}

	add_action('wp_enqueue_scripts', 'cinema_theme');
	...
	function films_css()
	{
		wp_enqueue_style(
			'films_css',
			get_template_directory_uri() . '/films.css',
			array(),
			'1.0.0',
			'all'
		);
	...
	add_action('wp_enqueue_scripts', 'films_css');
	...
	function register_cpt_film()
	{
		register_post_type(
			...
			[
				...
				'labels' => ['name' => 'Фильмы', 'singular_name' => 'Фильм']
			]
		);
	}
